    ...
    for (i in program) {
        ...
        if(line.trim().startsWith("goto_if")) {
            ...
            if (labelName.startsWith("*")) {
                ...
                error("Label " + labelName + " is referenced without a *", "SyntaxError", l, null, null, hint = "Add a * before the label name", hint_quote = "goto_if *" + labelName);
            }
        }
        //if the line is a call to a label, add it to the used labels list
            ...
            if (labelName.startsWith("*")) {
                ...
                error("Label " + labelName + " is referenced without a *", "SyntaxError", l, null, null, hint = "Add a * before the label name", hint_quote = "goto *" + labelName);
            }
        } 
        if (line.trim().startsWith("label")) {
            ...
            if (labelName.startsWith("*")) {
                ...
                error("Label " + labelName + " is referenced without a *", "SyntaxError", l, null, null, hint = "Add a * before the label name", hint_quote = "label *" + labelName);
            }
        }
        ...
        for (label in templabels) {
            if (line.startsWith(label)) {
                error("Label " + label + " is referenced without a *", "SyntaxError", l, null, null, hint = "Add a * before the label name", hint_quote = "*" + label);
                error_throw = true;
            }
        }
    }
...
//create a function to determine if each label is reached in the program or if it's skipped
function reachable(program) {
    //split the program into lines
    program = program.split("\n");
    //store labels above main
}
